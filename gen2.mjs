...
for (const j of JSON.parse(process.argv[2])) {
  ...
  if (!row || !row.isActive || row.isDeprecated) { console.log(`SKIP ${j.model}`); continue; }
  const endpoint = row.endpoint || row.providerModelId || row.modelId;
  const w = await getWallet(USER);
  if (w.available < row.creditsCost) { console.log(`SKIP ${j.model} (funds)`); continue; }
  ...
  console.log(`QUEUED ${j.tool} ${row.modelId} ${row.creditsCost}cr gen=${gen.id}`);
}
process.exit(0);
