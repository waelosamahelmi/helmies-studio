...
export default function AudioStudio() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/generate/audio", {
        ...
        body: JSON.stringify({ ...params, _modelId: model.id, endpoint: model.endpoint }),
      });
  ...
  const renderField = (key, field) => {
      ...
      return (
        <div key={key}>
          ...
          <button className="btn btn-secondary" onClick={() => fileRef.current?.click()}>
            {params[key] ? "Audio loaded ✓" : `Upload ${field.title}`}
          </button>
        </div>
      );
    }
    if (field.type === "enum") {
      return (
        <select key={key} className="field-select" value={params[key] || field.default} onChange={(e) => setParams((p) => ({ ...p, [key]: e.target.value }))}>
          {field.enum.map((opt) => (
    ...
    if (field.type === "int" || field.type === "number") {
      return (
        <input
          ...
          type="number"
          value={params[key] ?? field.default ?? 30}
          min={field.minValue}
          max={field.maxValue}
      ...
      <div className="studio-panel__right">
        ...
        {loading && (
          <div className="studio-loading">
            ...
            <p>Generating audio... This may take a minute.</p>
          </div>
        )}
        {result && result.url && (
        ...
        {!loading && !result && !error && (
          <div className="studio-empty">
            <div className="studio-empty__icon"><IconMusic /></div>
            <h3>Audio Studio</h3>
            <p>Generate music, voice, and sound effects with AI.</p>
          </div>
        )}
      </div>
    </div>
