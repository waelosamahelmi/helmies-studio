    ...
    <div className="studio-panel">
      <div className="studio-panel__left">
        ...
        <div className="field-group">
          ...
          <textarea className="field-textarea" placeholder="A cinematic drone shot of a futuristic city at sunset..." value={prompt} onChange={(e) => setPrompt(e.target.value)} rows={4} />
        </div>
        {model.aspectRatios && (
          <div className="field-group">
        ...
        {model.durations && (
          <div className="field-group">
            ...
            <div className="field-pills">
              {model.durations.map((d) => <button key={d} className={`pill ${duration === d ? "pill--active" : ""}`} onClick={() => setDuration(d)}>{d}s</button>)}
            </div>
          </div>
        )}
        <div className="field-group">
          ...
          <button className="btn btn-secondary" onClick={() => fileRef.current?.click()}>{imageUrl ? "Image loaded ✓" : "Upload start frame"}</button>
          {imageUrl && <button className="btn btn-ghost" onClick={() => setImageUrl("")}>Remove</button>}
        </div>
        <button className="btn btn-primary btn-lg w-full" onClick={handleSubmit} disabled={loading || (!prompt.trim() && !imageUrl)}>
          {loading ? "Generating video..." : <>Generate Video<span className="btn__icon"><IconBolt /></span></>}
        </button>
      </div>
      <div className="studio-panel__right">
        ...
        {!loading && !result && !error && <RichIdle tool="video" icon={IconVideo} title="Video Studio" description="Enter a prompt to generate AI video clips." />}
      </div>
    </div>
  );
}
