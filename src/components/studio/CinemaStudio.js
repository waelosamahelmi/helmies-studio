...
export default function CinemaStudio() {
  ...
  const buildPrompt = () => {
    return [prompt, camera.prompt, lens.prompt, focal.prompt, aperture.prompt].filter(Boolean).join(", ");
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/generate/cinema", {
        method: "POST",
  ...
  return (
    <div className="studio-panel">
      <div className="studio-panel__left">
        ...
        <div className="field-group">
          <label className="field-label">Reference Image (optional — switches to edit mode)</label>
        ...
        <button className="btn btn-primary btn-lg w-full" onClick={handleSubmit} disabled={loading || !prompt.trim()}>
          {loading ? "Generating..." : <>Generate <span className="btn__icon"><IconBolt /></span></>}
        </button>
      </div>
      <div className="studio-panel__right">
        ...
        {result && result.url && (
          <div className="studio-result">
            <img src={result.url} alt="Cinematic" className="studio-result__img" />
        ...
        {!loading && !result && !error && (
          <div className="studio-empty"><div className="studio-empty__icon"><IconCamera /></div><h3>Cinema Studio</h3><p>Pro camera controls for cinematic AI images.</p></div>
        )}
      </div>
    </div>
  );
