// Helmies Studio — reading a screenplay into scenes and shots.
//
// Extracted from src/app/api/projects/[id]/breakdown/route.js so the AGENT
// can run a breakdown too. It was a route-private function, which meant the
// only way to turn a script into a shot list was for a human to press the
// button — the agent could write a screenplay and then had nowhere to put
...
/* TWO PASSES.
   ...
   hoped for. */
export async function readScreenplay(script, onProgress, { limits = null, keepIndexes = new Set() } = {}) {
...
export async function runBreakdown({ projectId, userId, script, settings, replace, keepSceneIds = [] }) {
